...
export function MyFavoritesPage() {
  ...
  const loadMaterials = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, 'לא ניתן לטעון את המועדפים.'))
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- fetch favorites on mount
    void loadMaterials()
  }, [loadMaterials])

  async function handleRemoveFavorite(material: MaterialDto) {
    ...
    try {
      ...
      setActionError(getApiErrorMessage(err, 'לא ניתן להסיר מהמועדפים.'))
    } finally {
      setBusyAction(null)
    }
  }
  ...
  return (
    <section className="library-page">
      <div className="page-hero library-hero">
        <div>
          <span className="eyebrow">רשימה אישית</span>
          <h1>המועדפים שלי</h1>
          <p className="page-description">
            חומרים מאושרים ששמרת לגישה מהירה. המועדפים נפרדים מלייקים ונשמרים רק עבורך.
          </p>
        </div>
        <div className="library-summary" aria-label="סיכום מועדפים">
          <span>
      ...
      {loading && <p className="empty-state">טוען מועדפים...</p>}
      {error && <p className="error-text">{error}</p>}
      {actionError && <p className="error-text">{actionError}</p>}
      {!loading && !error && materials.length === 0 && (
        <p className="empty-state">עדיין לא שמרת חומרים למועדפים.</p>
      )}
      {!loading && !error && materials.length > 0 && visibleMaterials.length === 0 && (
        <p className="empty-state">לא נמצאו מועדפים שמתאימים לסינון הנוכחי.</p>
      )}
                ...
                tools={
                  <>
                    ...
                    <IconButton
                      ...
                      onClick={() => void handleRemoveFavorite(material)}
                    >
                      <span aria-hidden="true">★</span>
                    </IconButton>
                    <IconButton
